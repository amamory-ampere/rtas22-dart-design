...
data_t *sub_A_in, *sub_B_in, *sub_C_out;
...
int main (int argc, char **argv)
{
	...
	// Bind with HW-memcpy having hw-id 101 - sub
	retval = fred_bind(fred, &hw_ip_sub, 101);
	if (retval) {
		printf("fred_bind failed for hw-task %u\n", 101);
		error_code = 1;
	}

	sub_C_out = fred_map_buff(fred, hw_ip_sub, 0);
	if (!sub_C_out) {
		printf("fred_map_buff failed on buff 0 for sub_C_out\n");
		error_code = 1;
	}
	sub_A_in  = fred_map_buff(fred, hw_ip_sub, 1);
	if (!sub_A_in) {
		printf("fred_map_buff failed on buff 1 for sub_A_in\n");
		error_code = 1;
	}
	sub_B_in  = fred_map_buff(fred, hw_ip_sub, 2);
	if (!sub_B_in) {
		printf("fred_map_buff failed on buff 2 for sub_B_in\n");
		error_code = 1;
	}
...
for (int i=0;i<REPETITIONS;++i){
	...
	init_vect(sub_A_in, scalar_in_A);
	init_vect(sub_B_in, scalar_in_B);
...
*/
	...
	// Call fred IP - sub
	retval = fred_accel(fred, hw_ip_sub);
	if (retval) {
		printf("fred_accel failed for hw-task %u\n", 101);
		error_code = 1;
	}	
	...
	//validate
	if (check_output(sub_C_out, ARRAY_SIZE,scalar_in_A-scalar_in_B) != 1){
		printf("SUB Mismatch!\n");
		error_code = 1;
	}else{
		printf("SUB Match!\n");
...
/*
	printf("Content of A[0:9]:\n");
	print_vect(sub_A_in, 10);
	printf("Content of B[0:9]:\n");
	print_vect(sub_B_in, 10);
	printf("Content of C[0:9]:\n");
	print_vect(sub_C_out, 10);
*/
	scalar_in_A+=2;
	scalar_in_B++;
